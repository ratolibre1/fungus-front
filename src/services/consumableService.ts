...
export const getConsumables = async (filters: ConsumableFilters = {}): Promise<ConsumablesResponse> => {
  ...
  if (filters.inStock !== undefined) queryParams.push(`inStock=${filters.inStock}`);
...
export const getConsumable = async (id: string): Promise<ConsumableResponse> => {
  const response = await fetch(`${API_URL}/consumables/${id}`, {
    method: 'GET',
    headers: getAuthHeaders()
  });

  const apiResponse = await handleApiResponse<Consumable>(response);
  return {
    success: apiResponse.success,
    data: apiResponse.data
  };
...
export const createConsumable = async (consumable: CreateConsumableRequest): Promise<ConsumableResponse> => {
...
export const updateConsumable = async (id: string, consumable: UpdateConsumableRequest): Promise<ConsumableResponse> => {
  const response = await fetch(`${API_URL}/consumables/${id}`, {
    method: 'PUT',
    headers: getAuthHeaders(),
    body: JSON.stringify(consumable)
...
export const deleteConsumable = async (id: string): Promise<{ success: boolean, data: Record<string, never> }> => {
  const response = await fetch(`${API_URL}/consumables/${id}`, {
    method: 'DELETE',
    headers: getAuthHeaders()
  });

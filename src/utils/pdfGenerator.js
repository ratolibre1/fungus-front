...
export const generateQuotationPDF = (quotation) => {
    ...
    const primaryGreen = [9, 147, 71];
    const lightGray = [245, 248, 245];
    ...
    if (typeof quotation.counterparty === 'object') {
        ...
        if (quotation.counterparty.rut)
            rightData.push(['RUT:', formatRUT(quotation.counterparty.rut)]);
        if (quotation.counterparty.email)
            rightData.push(['Email:', quotation.counterparty.email]);
        if (quotation.counterparty.phone)
            rightData.push(['Teléfono:', quotation.counterparty.phone]);
    }
    else {
